...
class ViewCrafter:
    def __init__(self, opts, gradio=False, 
                 setup_diffusion=True, device="cuda:0"):
        self.opts = opts
        self.device = device
    ...
    def run_diffusion(self, renderings, loss_guidance_fn, no_guidance=False):
        # guidance_images: [n_frames, 3, H, W]
        ...
        with torch.cuda.amp.autocast():
            ...
            batch_samples = image_guided_synthesis(self.diffusion, prompts, videos, self.noise_shape, self.opts.n_samples, self.opts.ddim_steps, self.opts.ddim_eta, \
                                self.opts.unconditional_guidance_scale, self.opts.cfg_img, self.opts.frame_stride, self.opts.text_input, self.opts.multiple_cond_cfg, \
                                self.opts.timestep_spacing, self.opts.guidance_rescale, condition_index, 
                                loss_guidance_fn, no_guidance)
    ...
    def setup_dust3r(self):
        self.dust3r = load_model(self.opts.dust3r_path, self.device)
