...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if not viewpoint_stack:
            use_project_cam = np.random.rand() > opt.project_cam_prob
            viewpoint_stack = scene.getProjectCameras().copy() if use_project_cam else scene.getTrainCameras().copy()
        ...
        # Loss
        if not use_project_cam: 
            gt_image = viewpoint_cam.original_image.cuda()
            Ll1 =  l1_loss_mask(image, gt_image)
            loss = ((1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)))
        else: 
            gt_image = viewpoint_cam.projected_image.cuda()
            mask = torch.tensor(viewpoint_cam.mask).cuda()
            Ll1 = l1_loss_mask(image, gt_image, mask)
            loss = opt.project_cam_weight * Ll1
